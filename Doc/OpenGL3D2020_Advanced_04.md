...
+        continue;
+      }
+      // 草を包む球状の衝突判定を作り、それが視錐台と衝突したら表示リストに登録.
+      // 草モデルは高さ約1mなので、球は地面から0.5m高い位置に作る.
+      // 草モデルの上の方が広がっていることと、地形に沿って変形することから、
+      // 球の半径は余裕を見て1.0mとする.
+      glm::vec3 p(x + 0.5f, 0, z + 0.5f);
+      p.y = heights[z * size.x + x] + 0.5f;
+      if (Collision::Test(frustum, Collision::Sphere{p, 1.0f})) {
